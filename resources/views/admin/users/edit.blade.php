...
@section("content")
        ...
        <div class="is-row align-middle">
            ...
            <div class="is-col text-right">
                <nav class="breadcrumb">
                    ...
                    <a href="{{ route("admin.users.index") }}">{{ __('piclommerce::admin.navigation_customers') }}</a>
                    <span>{{ __('piclommerce::admin.edit') }} {{ $data->firstname }} {{ $data->lastname }}</span>
                </nav>
            </div>
        </div>
    ...
    <div class="content-container">
        ...
        <form class="admin-form" method="post" action="{{ route('admin.users.update',['uuid' => $data->uuid]) }}" enctype="multipart/form-data">
            {{ csrf_field() }}
            @include("piclommerce::admin.users.form", compact('data'))
